// FUTURE: #48 — Codex provider module; re-enable when multi-vendor registry is implemented.
//
// This module is fully implemented but not wired into any CLI command yet.
// When re-enabling, also:
//   - Uncomment `pub mod codex;` in lib.rs
//   - Remove `#![cfg(any())]` from tests/codex.rs
//   - Change `CodexSession::snapshot()` to return `&CodexSessionSnapshot` instead
//     of cloning, to avoid an unnecessary allocation on every snapshot call.
//
// use std::fmt;
...
//     }
//
//     // FUTURE: #48 — when re-enabling, change return type to `&CodexSessionSnapshot`
//     // to avoid cloning on every snapshot call.
