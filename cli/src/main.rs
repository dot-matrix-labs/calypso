...
use calypso_cli::init::{
    HostInitEnvironment, detect_repo_status, refresh_workflows, run_init_interactive, RepoInitStatus,
};
...
fn run_init_status(cwd: &std::path::Path) {
    ...
    match calypso_cli::init::InitProgress::load(cwd) {
        Ok(Some(progress)) => {
            println!("Init state: {}", progress.current_step);
            println!("Completed steps:");
            for step in &progress.completed_steps {
                println!("  [x] {step}");
            }
